...
class CycleGANUpdater(chainer.training.updaters.StandardUpdater):
    ...
    def update_core(self):
        ...
        gen_sp_optimizer = self.get_optimizer("gen_sp")
        ...
        self.gen_sp.cleargrads()
        fake_bm = self.gen_bm(batch_b_n)
        fake_bs = self.gen_sp(batch_b_n)
        fake_am = self.gen_am(batch_a_n)
        fake_as = self.gen_sp(batch_a_n)
        fake_ax = fake_am * fake_as
        fake_bx = fake_bm * fake_bs
        fake_aa = self.gen_ma(fake_ax)
        fake_bb = self.gen_mb(fake_bx)
        fake_ab = self.gen_mb(fake_ax)
        fake_ba = self.gen_ma(fake_bx)
        fake_aba = self.gen_bm(fake_ab)
        fake_bab = self.gen_am(fake_ba)
        fake_abs = self.gen_sp(fake_ab)
        fake_bas = self.gen_sp(fake_ba)
        ...
        loss_cycbb = F.sqrt(F.mean_squared_error(fake_bb, batch_b))
        loss_cycaa = F.sqrt(F.mean_squared_error(fake_aa, batch_a))
        loss_cycbam = F.sqrt(F.mean_squared_error(fake_bab, fake_bm))
        loss_cycabm = F.sqrt(F.mean_squared_error(fake_aba, fake_am))
        loss_cycas = F.sqrt(F.mean_squared_error(fake_bs, fake_bas))
        loss_cycbs = F.sqrt(F.mean_squared_error(fake_as, fake_abs))
        gloss = loss_ganba + loss_ganab + (loss_cycaa + loss_cycbb + loss_cycbam + loss_cycabm+ loss_cycas + loss_cycbs) * 10
        gloss.backward()
        chainer.report({"G_AB__GAN": loss_ganab,
                        "G_BA__GAN": loss_ganba,
                        "G_AA__CYC": loss_cycaa,
                        "G_BB__CYC": loss_cycbb,
                        "G_AS__CYC": loss_cycas,
                        "G_BS__CYC": loss_cycbs,
                        "G_ABA_CYC": loss_cycabm,
                        "G_BAB_CYC": loss_cycbam})
        gen_ba_optimizer.update()
        gen_ab_optimizer.update()
        gen_ba_optimizer2.update()
        gen_sp_optimizer.update()
