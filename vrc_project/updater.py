...
class CycleGANUpdater(chainer.training.updaters.StandardUpdater):
    ...
    def update_core(self):
        ...
        batch_a_n = noise_put(_xp, batch_a, 0.00)
        batch_b_n = noise_put(_xp, batch_b, 0.00)
        ...
        for _ in range(1):
            ...
            y_af2 = self.disa(batch_b_n)
            y_bf2 = self.disb(batch_a_n)
            ...
            loss_d_a = F.mean_squared_error(y_af, y_label_z) *0.25+ F.mean_squared_error(y_af2, y_label_z) *0.25+F.mean_squared_error(y_at, y_label_o) *0.5
            loss_d_b = F.mean_squared_error(y_bf, y_label_z) *0.25+ F.mean_squared_error(y_bf2, y_label_z) *0.25+F.mean_squared_error(y_bt, y_label_o) *0.5
...
def noise_put(_xp, x, stddev):
    ...
    x_s = x +_xp.random.randn(noise_shape[0], 1, noise_shape[2], noise_shape[3]) * stddev
