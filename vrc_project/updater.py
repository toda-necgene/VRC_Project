...
class CycleGANUpdater(chainer.training.updaters.StandardUpdater):
    ...
    def update_core(self):
        ...
        batch_bn = batch_b * (_xp.random.randn(batch_a.shape[0], 1, batch_a.shape[2], 1).astype(_xp.float32) * 0.002 * rate + _xp.ones([batch_a.shape[0], 1, 1, 1]))
        ...
        y_bt = self.disb(batch_bn)
        y_label_T = _xp.zeros(y_af.shape, dtype="float32")
        y_label_F = _xp.ones(y_bf.shape, dtype="float32")
        loss_d_af = F.mean_squared_error(y_af, y_label_F) * 0.5
        loss_d_bf = F.mean_squared_error(y_bf, y_label_F) * 0.5
        loss_d_ar = F.mean_squared_error(y_at, y_label_T) * 0.5
        loss_d_br = F.mean_squared_error(y_bt, y_label_T) * 0.5
        ...
        loss_ganab = F.mean_squared_error(y_fake_ab, y_label_T) * 0.5
        loss_ganba = F.mean_squared_error(y_fake_ba, y_label_T) * 0.5
