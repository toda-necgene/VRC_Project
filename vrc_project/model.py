...
class Generator(chainer.Chain):
    ...
    def __init__(self, chs=256, layers=9):
        ...
        for i in range(layers):
            self.add_link("0"+str(i+1)+"_r_0", L.Convolution2D(chs, chs, (11, 1), pad=(5, 0), initialW=w_init).add_hook(spn()))
            self.add_link("0"+str(i+1)+"_r_1", L.Convolution2D(chs, 1, 1, initialW=w_init).add_hook(spn()))
        self.add_link("98_d_0", L.Deconvolution2D(chs, 1, (4, 1), stride=(4, 1), initialW=w_init).add_hook(spn()))
