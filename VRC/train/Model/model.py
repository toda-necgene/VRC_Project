...
class Model:
    def __init__(self,debug):
        self.experiment=Experiment("Wave2Wave_train_test")
        self.gf_dim=64
        self.depth=7
        self.batch_size=110
        self.dataset_name="wave2wave ver0.2"
        #in=25
        a=2
        self.dilations=[]
    ...
    def build_model(self):
        ...
        with tf.variable_scope('wavenet'):
            ...
            with tf.variable_scope('postprocessing'):
                ...
                current['bias']=bias
                l2ls.append(bias)
                bias = tf.get_variable("bias2", [1],initializer=tf.constant_initializer(0.0))
                l2ls.append(bias)
                current['bias2']=bias
...
#         lo=-tf.reduce_sum(target*tf.log(logit+eps))/self.batch_size
        ...
        for epoch in range(1,100):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                for t in range(times):
                    target=np.reshape(batch_images[:,0,t],(self.batch_size,1,1))
                    resorce=np.reshape(batch_images[:,1,max(0,t-self.in_put_size[2]):t],(self.batch_size,1,-1))
                    r=max(0,self.in_put_size[2]-t)
                    if r>0:
                        resorce=np.pad(resorce,((0,0),(0,0),(0,r)),'constant')
                    # Update G network
                    ...
                    cur_res=cur_res[:,:,1:1+self.in_put_size[2]]
    ...
    def decode(self,in_puts):
        ...
        return tf.to_float(inputs*32767.0)
