...
def generator(ten,reuse,train):
    ...
    chs_enc=[32,64]
    chs_dec=[32,16]
    ten = tf.layers.conv2d(ten[:,:,:513,:], 16, [9, 3], [7, 1], padding="VALID",
                            kernel_initializer=tf.truncated_normal_initializer(stddev=math.sqrt(2.0 / 3/9/8)),
                            use_bias=False,
    ...
    for i in range(times):
        ...
        tenA = tf.layers.batch_normalization(tenA, axis=3, training=train, trainable=True, reuse=reuse,
                                             name="dec_bn1_" + str(i))
        tenA = tf.nn.relu(tenA)
    sds=tf.layers.conv2d(inp[:,:,-1:,:], 64, [9, 1], [7, 1], padding="VALID",
                            kernel_initializer=tf.truncated_normal_initializer(stddev=math.sqrt(2.0 / 3/9/8)),
                            use_bias=False,
                            data_format="channels_last", reuse=reuse, name="res_conv_C_pre_")
    tenB=ten*sds
    for i in range(times):

        tenB=deconve_with_ps(tenB,[1,1],chs_dec[i],reuse,"dec_2_"+str(i),False)
    ...
    tenB = tf.layers.conv2d_transpose(tenB, 1, kernel_size=[9, 1], strides=[7, 1], padding="VALID",
                                      kernel_initializer=tf.truncated_normal_initializer(stddev=0.002), use_bias=True,
                                      data_format="channels_last", reuse=reuse, name="last_conv2")
    tenB = tf.transpose(tenB, [0, 1, 3, 2])
    tenB = tf.layers.dense(tenB, 1, kernel_initializer=tf.truncated_normal_initializer(stddev=math.sqrt(2.0)),
                           use_bias=False, reuse=reuse, name="dense")
    tenB = tf.tile(tf.transpose(tenB, [0, 1, 3, 2]),(1,1,513,1))

    return tf.concat([tenA,tenB],axis=2)[:,:,:514,:]
