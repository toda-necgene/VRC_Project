    ...
    for i in range(depth):
        ...
        if i!=0:
            ten = tf.layers.batch_normalization(ten, axis=-1, training=True, trainable=True, reuse=reuse,
                                                name="disc_bn_" + str(i))
        ten = tf.layers.conv2d(ten, chs[i], kernel_size=[3,5], strides=[1,4], padding="SAME",kernel_initializer=tf.truncated_normal_initializer(stddev=math.sqrt(2.0/3/5/chs[i])),use_bias=True, data_format="channels_last",name="disc_"+str(i),reuse=reuse)
...
def generator(ten,reuse,train):
    ...
    ten=tf.layers.conv2d(ten, 64, [1, 3], [1, 3], padding="SAME",
                           kernel_initializer=tf.truncated_normal_initializer(stddev=math.sqrt(2.0/3/128)), use_bias=True,
    ...
    ten = tf.layers.batch_normalization(ten, axis=3, training=train, trainable=True, reuse=reuse,
                                         name="dec_bn_1_11")
    ten = deconve_with_ps(ten, [1, 3], 8, reuse, "dec_31", True)
    ten = tf.nn.leaky_relu(ten)
